...
public class RepeatedParameters extends AppCompatActivity {
    ...
    public void todaysSessionsNav(View v){
        ...
        Task newTask = new RepetitiveTask(name, estimatedDifficulty, howOften);

        loadData();
        sessionModels.add(new Session(newTask.getName(), sessionModels.get(sessionModels.size()-1).getEndTime() + Constants.BUFFER_TIME, sessionModels.get(sessionModels.size()-1).getEndTime() + Constants.BUFFER_TIME + newTask.getEstimatedTime()/newTask.getDifficulty(), "Repeated"));
        saveData(v);
